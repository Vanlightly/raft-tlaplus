...
\* the number of votes a server needs to win an election
CONSTANTS ElectionQuorumSize

\* the number of copies of an entry for it to be committed
CONSTANTS ReplicationQuorumSize
...
BecomeLeader(i) ==
    /\ state[i] = Candidate
    /\ Cardinality(votesGranted[i]) >= ElectionQuorumSize
...
AdvanceCommitIndex(i) ==
    ...
    /\ LET \* The set of servers that agree up through index.
           ...
           agreeIndexes == {index \in 1..Len(log[i]) : 
                                Cardinality(Agree(index)) >= ReplicationQuorumSize}
