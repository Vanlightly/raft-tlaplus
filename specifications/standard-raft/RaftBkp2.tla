...
\* Note1: to prevent infinite cycles, empty 
...
\* The server and log index point to an OldNewConfigCommand
\* which has no corresponding NewConfigCommand yet.
\* Basically, the leader needs to send out a NewConfigCommand now.
...
\* Returns a members set that may or may not have changed.
UpdateMemberSet(i, m, newLog) ==
    \* if there is at least one reconfig command in the new
    \* log, then take the most recent and use the member set of that command 
    IF newLog[i] # <<>> /\ \E index \in DOMAIN newLog[i] : 
    ...
    \* else there is no reason to change the member set
    ELSE members[i]
...
InitServerVars(initLeader, cluster) == 
    /\ currentTerm = [i \in Server |-> 0]
    /\ state       = [i \in Server |-> IF i = initLeader
                                       THEN Leader
                                       ELSE NotMember]
...
Init == LET cluster    == CHOOSE s \in SUBSET Server :
            ...
            initLeader == CHOOSE i \in cluster : TRUE
        IN
            /\ messages = [m \in {} |-> 0]
            /\ members = [i \in Server |->
                            IF i = initLeader
...
\* Note that this specification sets the nextIndex to be 1 past the end of
\* the leaders log (as per the Raft paper). However, the leader could 
\* set the next index to the highest common index + 1 if it were to receive
\* the last index of the followers in their RequestVote responses.
BecomeLeader(i) ==
    \* enabling conditions
    /\ state[i] = Candidate
...
\*
\* When sending the entry of index 1, the member set is included in the message.
AppendEntries(i, j) ==
    \* enabling conditions
    /\ i /= j
    /\ state[i] = Leader
    /\ j \in members[i]              \* j is a member
...
\* - addMembers or removeMembers should not be an empty set
...
AppendOldNewConfigToLog(i) ==
    ...
    /\ \E addMembers \in SUBSET Server, removeMembers \in SUBSET Server :
        /\ Cardinality(removeMembers) < Cardinality(members[i])
        /\ addMembers \intersect members[i] = {}
        /\ removeMembers \intersect members[i] = removeMembers
        /\ \/ addMembers # {}
           \/ removeMembers # {}
        /\ ~HasPendingConfigCommand(i)
        \* state changes
        /\ LET entry == [command |-> OldNewConfigCommand,
                         term    |-> currentTerm[i],
                         value   |-> [id      |-> reconfigCtr, \* id for simpler temporal formulae
                                      old     |-> members[i],  \* the old configuration members
                                      new     |-> (members[i] \ removeMembers) \union addMembers, \* the new configuration members
                                      members |-> members[i] \union addMembers]] \* the joint consensus members
               newLog == Append(log[i], entry)
           IN  /\ log' = [log EXCEPT ![i] = newLog]
