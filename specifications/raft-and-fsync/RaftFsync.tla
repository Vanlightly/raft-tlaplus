...
\* The highest index which has been synced to stable storage
VARIABLE fsyncIndex
logVars == <<log, commitIndex, fsyncIndex>>
...
vars == <<messages, serverVars, candidateVars, leaderVars, logVars, fsyncIndex, auxVars>>
view == <<messages, serverVars, candidateVars, leaderVars, logVars, fsyncIndex >>
...
\* The log gets truncated back to the fsyncIndex
Restart(i) ==
    ...
    /\ log'            = [log EXCEPT ![i] = 
                                 IF fsyncIndex[i] = 0
                                 THEN <<>>
                                 ELSE IF Len(@) > 0 /\ Len(@) > fsyncIndex[i]
                                      THEN SubSeq(@, 1, fsyncIndex[i])
                                      ELSE @]
    /\ UNCHANGED <<messages, currentTerm, votedFor, fsyncIndex, 
                   acked, electionCtr>>
...
Timeout(i) ==
    ...
    /\ UNCHANGED <<acked, messages, leaderVars, logVars, restartCtr>>

\* ACTION: RequestVote
\* Candidate i sends j a RequestVote request.
RequestVote(i, j) ==
    /\ state[i] = Candidate
    /\ i # j \* it voted for itself already
...
AppendEntries(i, j) ==
    ...
    /\ LET prevLogIndex == nextIndex[i][j] - 1
       ...
       IN 
          /\ IF LeaderFsyncBeforeAppendEntries
             THEN fsyncIndex[i] >= lastEntry
             ELSE TRUE
...
ClientRequest(i, v) ==
    ...
    /\ UNCHANGED <<messages, serverVars, candidateVars,
                   leaderVars, commitIndex, electionCtr, restartCtr, fsyncIndex>>
...
AdvanceCommitIndex(i) ==
    ...
    /\ UNCHANGED <<messages, serverVars, candidateVars, leaderVars, log, 
                   electionCtr, restartCtr, fsyncIndex>>

\* ACTION: AdvanceFsyncIndex ---------------------------------------
\* Advances the index that indicates data persisted to stable storage
AdvanceFsyncIndex(i) ==
    /\ fsyncIndex[i] < Len(log[i])
    /\ fsyncIndex' = [fsyncIndex EXCEPT ![i] = @ + 1]
    /\ UNCHANGED <<messages, serverVars, candidateVars, leaderVars, log, 
                   auxVars, commitIndex>>
              ...
              /\ LET new_log == IF CanAppend(m, i)
                 ...
                 IN
                    ...
                    /\ commitIndex' = [commitIndex EXCEPT ![i] =
                                              m.mcommitIndex]
                    /\ IF FollowerFsyncBeforeReply
                       THEN fsyncIndex' = [fsyncIndex EXCEPT ![i] = Len(new_log[i])]
                       ELSE UNCHANGED fsyncIndex                                             
                    /\ log' = new_log
                    /\ Reply([mtype           |-> AppendEntriesResponse,
                              mterm           |-> currentTerm[i],
                              msuccess        |-> TRUE,
...
Next == 
        ...
        \/ \E i \in Server : AdvanceFsyncIndex(i)
        \/ UpdateTerm
        \/ HandleRequestVoteRequest
        \/ HandleRequestVoteResponse
        \/ RejectAppendEntriesRequest
