...
In addition to the pull not push mechanism, the main differences are:
- after a server votes for a peer it straight away assumes that 
  the peer will become the new leader of that term and starts sending 
  PullEntries requests to that peer. The peer (the potential leader) 
  will reject these requests until it becomes leader, putting in the 
  response that it is pending leadership. The follower will keep 
  retrying until the peer becomes leader or another election occurs.
- when a leader is elected, for any servers that it didn't get a vote
  from, it sends a LeaderNotifyRequest so that these servers can learn
  of the new leader.

The follower includes its last entry index and term in its PullEntries
requests. If the last entry of the follower is inconstitent with the
log of the leader then the leader will send back the highest common
entry the two servers share. The follower truncates its log to that entry
and resumes sending PullEntries requests.

The leader advances the commit index as normal.

In this specification we do not explicitly model the PullEntries retries
while a candidate waits to become leader. We get the same outcome by 
only allowing a server to respond to PullEntries requests once
it has become leader. The implementation would have the same PullEntry request
being sent over and over but this TLA+ specification simply keeps the
PullEntry request as receivable until the candidate becomes leader. This
way we keep the spec simpler and non-infinite state space.
*)
...
\* Nil if it hasn't voted for any. In this pull model, when
\* a server votes for another, it presumes it becomes leader
\* until it discovers otherwise
VARIABLE leader
serverVars == <<currentTerm, state, leader>>
...
\* It loses everything but its currentTerm, leader and log.
Restart(i) ==
    ...
    /\ UNCHANGED <<messages, currentTerm, leader, log, acked, electionCtr>>
...
RequestVote(i) ==
    ...
    /\ leader' = [leader EXCEPT ![i] = i] \* votes for itself
...
BecomeLeader(i) ==
    ...
    /\ SendMultiple(
          {[mtype            |-> LeaderNotifyRequest,
            mterm            |-> currentTerm[i],
            msource          |-> i,
            mdest            |-> j] : j \in Server \ votesGranted[i]})
    /\ UNCHANGED <<currentTerm, leader, 
...
\* A follower learns of a new leader that is had not voted for.
LearnOfLeader ==
    \E m \in DOMAIN messages :
        /\ ReceivableMessage(m, LeaderNotifyRequest, EqualTerm)
        /\ LET i     == m.mdest
               j     == m.msource
           IN /\ leader' = [leader EXCEPT ![i] = j]
              /\ Discard(m)
        /\ UNCHANGED <<currentTerm, state, log, candidateVars,
                       leaderVars, commitIndex, auxVars>>

\* ACTION: SendPullEntriesRequest ----------------------------------------
\* Follower i sends leader j a PullEntries request.
\* Note that j may not actually be a leader yet, only a candidate.
...
\* as a follower can't send an identical pull entries request again
\* (due to how the state space is constrained in this spec).
\* This issue is particular to this pull model.
              ...
              /\ LET newMatchIndex == [matchIndex[i] EXCEPT ![j] = m.mlastLogIndex]
                 ...
                 IN
                    ...
                    /\ Reply([mtype           |-> PullEntriesResponse,
                              ...
                              msource         |-> i,
                              mdest           |-> j], m)
                    /\ UNCHANGED <<candidateVars, currentTerm, 
